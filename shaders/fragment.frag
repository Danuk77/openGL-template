...
vec3 calculate_point_light(Point_light light, vec3 normal, vec3 fragPos, vec3 viewDir);
vec3 calculate_diffuse_light(vec3 light_dir, vec3 normalised_normal, vec3 diffuse_color);
vec3 calculate_specular_light(vec3 light_dir, vec3 normalised_normal, vec3 specular_light);
...
vec3 calculate_directional_light(Directional_light light, vec3 normal){
    ...
    vec3 diffuse_light = calculate_diffuse_light(light_direction, normalised_normal, light.diffuse);
    vec3 specular_light = calculate_specular_light(light_direction, normalised_normal, light.specular);
...
vec3 calculate_point_light(Point_light light, vec3 normal, vec3 fragPos, vec3 viewDir){
    ...
    vec3 diffuse = calculate_diffuse_light(light_direction, normalised_normal, light.diffuse);
    vec3 specular_light = calculate_specular_light(light_direction, normalised_normal, light.specular);
...
vec3 calculate_diffuse_light(vec3 light_dir, vec3 normalised_normal, vec3 diffuse_color){
    ...
    vec3 diffuse_intensity = diffuse_color * (diffuse_color * corrected_diffuse_intensity_multiplier);

    return diffuse_intensity;
}

vec3 calculate_specular_light(vec3 light_dir, vec3 normalised_normal, vec3 specular_color){
    ...
    vec3 specular_light_intensity = specular_color * specular_light_intensity_multiplier * specular_map_intensity;
